...
fn is_left_of(tree: *Triangulation, point_id: PointID, segment_point_1_id: PointID, segment_point_2_id: PointID) bool {
    ...
    const mul1: f32 = (b[0] - a[0]) * (point[1] - a[1]);
    const mul2: f32 = (b[1] - a[1]) * (point[0] - a[0]);
    const d: f32 = mul1 - mul2;
    return (d > 0);
}

fn point_is_above(tree: *Triangulation, lhs: PointID, rhs: PointID) bool {
    const lhs_yval = tree.points[lhs][1];
    const rhs_yval = tree.points[rhs][1];
...
fn push_triangle_if_acute(tree: *Triangulation, point: PointID, axis1: PointID, axis2: PointID, context: anytype, emit: fn (context: @TypeOf(context), point: Point) void) bool {
    const normalized_x1 = tree.points[point][0] - tree.points[axis1][0];
    const normalized_y1 = tree.points[point][1] - tree.points[axis1][1];
    const normalized_x2 = tree.points[point][0] - tree.points[axis2][0];
    const normalized_y2 = tree.points[point][1] - tree.points[axis2][1];
    const is_acute = @abs(std.math.atan2(normalized_y1, normalized_x1) - std.math.atan2(normalized_y2, normalized_x2)) < std.math.pi;
...
pub fn create_polygon(
    ...
    points: []const Point,
    context: anytype,
    emit: fn (context: @TypeOf(context), point: Point) void,
) !void {
